...
export const LABEL_JP = {
  ...
  atk_start:"攻撃開始", atk1:"攻撃1", atk2:"攻撃2", atk3:"攻撃3",
...
-各コマのポーズはテンプレートのマネキンに正確に従う
-各コマのポーズは以下（左から）
 -1段目:待機/横向き/歩き1（画面手前の足を画面右に出す。頭の角度は「横向き」と合わせる）/歩き2左足前（歩き1とは逆の足。画面手前の足を画面左に出す。頭の角度は「横向き」と合わせる）/しゃがみ
 -2段目:攻撃構え/攻撃1右腕パンチ（画面手前の腕を画面右に出す。背中を見せる）/攻撃2左腕パンチ（攻撃1とは逆の腕。画面の奥を画面右に出す。胸を見せる）/攻撃3右腕アッパー
 -3段目:ジャンプ上り/ジャンプ下り/ダメージ/吹っ飛び/ダウン
 -2段目の右端1コマは空(緑のまま)
