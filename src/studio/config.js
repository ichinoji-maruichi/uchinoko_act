...
渡されたキャラクターイラストと参照データのスプライト表のテンプレートを元にスプライトシートを作成する。
ゲーム制作にそのまま利用できる、アンチエイリアスのないハードエッジなピクセルアートを提供してください。

キャラクターに大きな羽、大きな尻尾、大きな耳などの突出したパーツがある場合は 'sprite-template-b.png' をテンプレートとして使用します。それ以外の場合は 'sprite-template-a.png' を使用します。

テンプレートのマネキンとまったく同じ配置・同じポーズ・同じ頭の向き・同じ緑背景(RGB 0,255,34)・同じ頭身で、渡されたイラストのキャラクターをデフォルメしたピクセルアートのスプライト表を作ってください。

デザインテイスト:
今風の高解像度で、ディテールが細かく表現された、かわいい感じのピクセルアートスタイルを採用してください。

テクニカルルック:
'hard edges'、'no anti-aliasing'、'indexed color look' を維持し、背景との境界をハードエッジにします。

厳守事項:
-ドットキャラのデザインテイスト:今風の解像度が高く小物なども細かく表現され、かわいい感じのテイスト。
-5列×3行のグリッド。各コマの位置・大きさはテンプレートと完全に一致させる
-各コマのキャラの位置、向き、ポーズはテンプレートのマネキンに正確に従うこと
-各コマのポーズは以下（左から）
 -1段目:待機/横向き/歩き1（画面手前の足を画面右に出す。頭の角度は「横向き」と合わせる）/歩き2左足前（歩き1の次の1歩。歩き1とは逆の足、画面手前の足を画面左に出す。頭の角度は「横向き」と合わせる）/しゃがみ
 -2段目:攻撃構え/攻撃1左腕パンチ（画面奥の腕を画面右に出す。胸を見せる）/攻撃2右腕パンチ（攻撃1のパンチと逆の腕でパンチ。頭の向きは攻撃1と揃える。背中を見せる。画面の手前の腕を画面右に出す）/攻撃3左ハイキック（画面奥の足でキック。胸を見せる）/右ジャンプアッパー（少し宙に浮きつつ画面手前の右腕を画面右斜め上へ突き上げるアッパー。頭は横向き）
 -3段目:ジャンプ上り/ジャンプ下り/ダメージ/吹っ飛び/ダウン（倒れ。処理の都合上、コマの上寄りに描く）
-背景は隙間なく純緑(0,255,34)で塗りつぶす。中間色を作らない。影やグラデ背景は入れない
-キャラの髪・服・色は1枚目に忠実に。全コマで同一人物を保つ
-手に武器や持ち物を持っている場合は無くして良い
